...
因为现在中低频更容易受宏观影响，不确定性越来越大，而高频衰减相对更缓慢。  
如果你还在做高频、盘口、逐笔、期权链路研究，那么数据本身依然是护城河的一部分，这个时候对 `TARDIS` 数据的需求只会更旺盛。

如果你不想直接承担 Tardis 官方高价订阅成本，可以从我们这里拿更低价的数据方案。

### 我们提供什么

我们有 `3` 个 Tardis API，期权、期货、现货各交易所数据都有。

本店的 API 是 `Solo 每月 1200 美元` 档位，合约、现货、期权所有历史数据和实时数据全部有，最高档位 API 能下载的内容这里都覆盖。
...
- `100 元 / 月起`
- 适合少量数据且需要日更，直接由我们持续维护更新

`🌹 整月租用高档 API`

- `1500 元 / 月`
- 适合需要大量数据的量化团队，相当于拿到一个 API 随便下载

### Deribit / Binance 特价数据

- 一年 `Deribit` 期权所有币种 `1T` 数据量：`200 元`
- 两年 `Deribit` 期权所有币种 `2T` 数据量：`350 元`
- 三年 `Deribit` 期权所有币种 `3T` 数据量：`480 元`
- `Binance` 合约 `BTC` 所有历史数据 `2T+`：`300 元`
- `Binance` 现货 `BTC` 所有历史数据：`180 元`

### 联系方式与说明

- 微信号：`15180066256`
- 需要下载数据的朋友可以直接搜索微信号添加
- `Tardis API` 价格高、储存成本也高，我们走的是低价薄利

备注：

- 加微信号 `15180066256` 付费和获取数据
- 加微信 `dreamsjtuai` 或 `a13479004101`，通过付款截图可获取本人 `1000 美金` 购买的 crypto 高频课程和代码

### 二维码与套餐截图

`TARDIS 数据微信二维码`

![TARDIS 数据微信二维码](./docs/assets/tardis-wechat-qr.jpg)

`TARDIS 套餐价格截图`

![TARDIS 套餐价格截图](./docs/assets/tardis-solo-plan.jpg)

`TARDIS API 服务说明截图`

![TARDIS API 服务说明截图](./docs/assets/tardis-service-detail.jpg)
